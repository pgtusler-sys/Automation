 ...
 *
 * In production, N8N triggers these functions on schedule.
 * This file can also be run directly for testing.
 */
...
export async function runEmailPipeline(): Promise<void> {
  ...
  for (const email of emails) {
    try {
      // Step 2: Generate and create draft reply
      ...
      // Step 3: Record in feedback store
      store.recordDraft({
        emailId: email.id,
      ...
      // Step 4: Route attachments if present
